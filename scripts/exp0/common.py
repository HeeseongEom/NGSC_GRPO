...
def load_config(path: str | Path = ROOT / "configs" / "exp0.yaml") -> dict:
...
def fingerprint(cfg: Mapping) -> str:
    ...
    for path in sorted((ROOT / "scripts" / "exp0").glob("*.py")):
...
def split_dir(cfg: Mapping, dataset: str, train_pairs: int) -> Path:
    return output_root(cfg) / "splits" / f"n{int(train_pairs)}" / dataset
...
def cache_dir(cfg: Mapping, dataset: str) -> Path:
    return output_root(cfg) / "cache" / cfg["method"] / dataset
...
def hard_reward(pair: Pair, actions: torch.Tensor) -> torch.Tensor:
    ...
    return 1.0 - prediction.float().mean((-2, -1))
...
def train_policy(
    ...
    for update in range(1, int(settings["updates"]) + 1):
        ...
        with torch.no_grad():
            ...
            rewards = torch.stack([hard_reward(pair, physical[index]) for index, pair in enumerate(batch)])
